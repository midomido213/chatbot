...
function question1(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'今回の予習は、順列に関する作問学習でした。'
  }).then(function(){
    log = '\r\n-----Q-----\r\n今回の予習は、順列に関する作問学習でした。\r\n';
    add_log_bot(log);
    botui.message.add({
      delay:1000,
      loading:true,
...
function question2(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'これまで学習してきた順列の範囲の中で、まだ理解ができていないと感じる箇所はありますか？'
  }).then(function(){
    log = '\r\n-----Q-----\r\nこれまで学習してきた順列の範囲の中で、まだ理解ができていないと感じる箇所はありますか？\r\n';
    add_log_bot(log);
    // ボタン表示
    return botui.action.button({
      autoHIde:false,
