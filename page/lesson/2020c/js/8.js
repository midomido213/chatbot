...
function ishiDb(targetScore, actualScore, satisfaction, reflection){
  var date = new Date(2020, 6, 10, 0, 0, 0, 0); // 月は0~11
...
function repoTime(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'第８回の授業内容の学習に要した時間を分単位で記入してね。'
  }).then(function(){
    log = '第８回の授業内容の学習に要した時間を分単位で記入してね。';
    addBot(log);
...
function repoStudy(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'eラーニング教材の第８回演習問題(20問)を全て回答しましたか？'
  }).then(function(){
    log = 'eラーニング教材の第８回演習問題(20問)を全て回答しましたか？';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:10,
...
function question3(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'今回学習したベクトル④（ベクトルの応用，空間ベクトル①）の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）'
  }).then(function(){
    log = '今回学習したベクトル④（ベクトルの応用，空間ベクトル①）の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:1000,
