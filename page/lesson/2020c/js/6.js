...
function ishiDb(targetScore, actualScore, satisfaction){
  var date = new Date(2020, 5, 26, 0, 0, 0, 0); // 月は0~11
...
function repoTime(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'第６回の授業内容の学習に要した時間を分単位で記入してね。'
  }).then(function(){
    log = '第６回の授業内容の学習に要した時間を分単位で記入してね。';
    addBot(log);
...
function repoStudy(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'eラーニング教材の第５回演習問題(7問)を全て回答しましたか？'
  }).then(function(){
    log = 'eラーニング教材の第５回演習問題(7問)を全て回答しましたか？';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:10,
          ...
          if(reExamCheckIs == '受験した'){
            botui.message.add({
              ...
              botui.message.add({
                delay:10,
                loading:true,
                content:'再テスト点数：' + correct2 + '点'
              }).then(function(){
                log += '再テスト点数：' + correct2 + '点';
                addBot(log);
                repoEnd();
              });
            });
...
function question3(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'今回学習したベクトル②（位置ベクトル）の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）'
  }).then(function(){
    log = '今回学習したベクトル②（位置ベクトル）の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:1000,
