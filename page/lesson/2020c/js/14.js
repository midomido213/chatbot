...
// status update
var update_url = '../api/statusUpdate13.php';

init();

// ishiのDB関数
function ishiDb(targetScore, actualScore, satisfaction, reflection){
  var date = new Date(2020, 7, 28, 0, 0, 0, 0); // 月は0~11
...
function repoTime(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'第１４回の授業内容の学習に要した時間を分単位で記入してね。'
  }).then(function(){
    log = '第１４回の授業内容の学習に要した時間を分単位で記入してね。';
    addBot(log);
...
function repoStudy(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'今回の予習（ベクトルの演習問題）を全問回答してきましたか？'
  }).then(function(){
    log = '今回の予習（ベクトルの演習問題）を全問回答してきましたか？';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:10,
  ...
  botui.message.add({
    ...
    botui.message.add({
      ...
      botui.message.add({
        delay:10,
        loading:true,
        content:'eラーニング回答状況：予習なし'
      }).then(function(){
        log += 'eラーニング回答状況：予習なし<br>';
        // correct = exam.reduce((a, x) => a+=x, 0);
        botui.message.add({
          delay:10,
          loading:true,
          content:'事前テスト点数：実施なし'
        }).then(function(){
          log += '事前テスト点数：実施なし';
          addBot(log);
          repoEnd();
...
function question2(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'情報基礎数学Cでこれまで学習してきた内容の理解度を選択してください。'
  }).then(function(){
    log = '情報基礎数学Cでこれまで学習してきた内容の理解度を選択してください。';
    addBot(log);
    botui.action.select({
      action:{
        placeholder:'学習満足度を選択',
...
function question3(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'これまで学習してきた中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）'
  }).then(function(){
    log = 'これまで学習してきた中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:1000,
...
function search(message){
  if((message.indexOf('和') != -1) || (message.indexOf('差') != -1) || (message.indexOf('実数倍') != -1) || (message.indexOf('積') != -1)){
    botui.message.add({
      delay:1000,
      loading:true,
      content:'行列の和・差・実数倍・積に関する補足説明動画が見つかりました！'
    }).then(function(){
      log = '行列の和・差・実数倍・積に関する補足説明動画が見つかりました！<br>';
      botui.message.add({
        delay:1000,
        loading:true,
        ...
        botui.message.add({
          ...
          log += 'https://www.youtube.com/embed/hCvlSF4i-RM<br>';
          botui.message.add({
            delay:1000,
            loading:true,
    ...
    botui.message.add({
      delay:1000,
      loading:true,
      content:'ケーリー・ハミルトンの定理に関する補足説明動画が見つかりました！'
    }).then(function(){
      log = 'ケーリー・ハミルトンの定理に関する補足説明動画が見つかりました！<br>';
      botui.message.add({
        delay:1000,
        loading:true,
        content:'動画を見ることで理解できるかもしれないので，見てみてください！'
        ...
        botui.message.add({
          ...
          log += 'https://www.youtube.com/embed/yGOtzeqB49Q<br>';
          botui.message.add({
            delay:1000,
            loading:true,
            content:'動画を見終わったらボタンを押してください．'
  ...
  }else if(message.indexOf('変換') != -1){
    botui.message.add({
      delay:1000,
      loading:true,
      content:'１次変換に関する補足説明動画が見つかりました！'
    }).then(function(){
      log = '１次変換に関する補足説明動画が見つかりました！<br>';
      botui.message.add({
        delay:1000,
        loading:true,
        content:'動画を見ることで理解できるかもしれないので，見てみてください！'
        ...
        botui.message.add({
          ...
          log += 'https://www.youtube.com/embed/u5MLbZrYMXM<br>';
          botui.message.add({
            delay:1000,
            loading:true,
            content:'動画を見終わったらボタンを押してください．'
    ...
    botui.message.add({
      ...
      botui.message.add({
        ...
        botui.message.add({
          ...
          log += 'https://www.youtube.com/embed/jwcHmtCnXa0<br>';
          botui.message.add({
            delay:1000,
            loading:true,
            content:'動画を見終わったらボタンを押してください．'
...
function ishi_reflection2(executing){
  botui.message.add({
    delay:500,
    loading:true,
    content:'あなたの第１４回授業の計画実施率は' + executing + '%でした。'
  }).then(function(){
    log = 'あなたの第１４回授業の計画実施率は' + executing + '%でした。';
    addBot(log);
    botui.message.add({
      delay:500,
      loading:true,
...
function end(){
  botui.message.add({
    ...
    botui.message.add({
      delay:1000,
      loading:true,
      content:'今回でチャットボットでの振り返りは終わりです。授業終了時にシステム利用アンケートがあるので、率直に回答をよろしくお願いします！'
    }).then(function(){
      log = '今回でチャットボットでの振り返りは終わりです。授業終了時にシステム利用アンケートがあるので、率直に回答をよろしくお願いします！';
      addBot(log);
