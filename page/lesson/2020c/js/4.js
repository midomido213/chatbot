...
function ishiDb(targetScore, actualScore, satisfaction){
  var date = new Date(2020, 5, 12, 0, 0, 0, 0); // 月は0~11
...
function repoTime(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'第４回の授業内容の学習に要した時間を分単位で記入してね。'
  }).then(function(){
    log = '第４回の授業内容の学習に要した時間を分単位で記入してね。';
    addBot(log);
...
function question3(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'これまで学習してきた「命題論理・集合」の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）'
  }).then(function(){
    log = 'これまで学習してきた「命題論理・集合」の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:1000,
