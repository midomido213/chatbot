...
// status update
var update_url = '../api/statusUpdate12.php';

init();

// ishiのDB関数
function ishiDb(targetScore, actualScore, satisfaction, reflection){
  var date = new Date(2020, 7, 12, 0, 0, 0, 0); // 月は0~11
...
function repoTime(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'第１２回の授業内容の学習に要した時間を分単位で記入してね。'
  }).then(function(){
    log = '第１２回の授業内容の学習に要した時間を分単位で記入してね。';
    addBot(log);
...
function repoStudy(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'eラーニング教材の第１２回演習問題(8問)を全て回答しましたか？'
  }).then(function(){
    log = 'eラーニング教材の第１２回演習問題(8問)を全て回答しましたか？';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:10,
...
function question3(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'今回学習した行列②（逆行列，連立1次方程式の解法，一次変換）の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）'
  }).then(function(){
    log = '今回学習した行列②（逆行列，連立1次方程式の解法，一次変換）の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:1000,
...
function ishi_reflection2(executing){
  botui.message.add({
    delay:500,
    loading:true,
    content:'あなたの第12回授業の計画実施率は' + executing + '%でした。'
  }).then(function(){
    log = 'あなたの第12回授業の計画実施率は' + executing + '%でした。';
    addBot(log);
    botui.message.add({
      delay:500,
      loading:true,
