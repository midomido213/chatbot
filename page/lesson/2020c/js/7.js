...
function ishiDb(targetScore, actualScore, satisfaction){
  var date = new Date(2020, 6, 3, 0, 0, 0, 0); // 月は0~11
...
function ishi_reflection(){
  ...
  request.open('GET', ishi_reflection_url + (lesson - 1));
  request.onload = function(){
    console.log(request.responseText);
    result = JSON.parse(request.responseText);
    demo(result.executing);
  }
  request.send();
}
...
function repoTime(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'第７回の授業内容の学習に要した時間を分単位で記入してね。'
  }).then(function(){
    log = '第７回の授業内容の学習に要した時間を分単位で記入してね。';
    addBot(log);
...
function repoStudy(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'eラーニング教材の第７回演習問題(6問)を全て回答しましたか？'
  }).then(function(){
    log = 'eラーニング教材の第７回演習問題(6問)を全て回答しましたか？';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:10,
    ...
    botui.message.add({
      ...
      botui.message.add({
        ...
        botui.message.add({
          delay:10,
          loading:true,
          content:'確認テスト点数：' + correct + '点'
        }).then(function(){
          log += '確認テスト点数：' + correct + '点<br>';
          botui.message.add({
            delay:10,
            loading:true,
            content:'事後テスト点数：' + correct2 + '点'
          }).then(function(){
            log += '事後テスト点数：' + correct2 + '点';
            addBot(log);
            repoEnd();
          });
        });
...
function question3(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'今回学習したベクトル③（ベクトル方程式）の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）'
  }).then(function(){
    log = '今回学習したベクトル③（ベクトル方程式）の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:1000,
...
function ishi_reflection2(executing){
  botui.message.add({
    delay:500,
    loading:true,
    content:'あなたの第7回授業の計画実施率は' + executing + '%でした。'
  }).then(function(){
    log = 'あなたの第7回授業の計画実施率は' + executing + '%でした。';
    addBot(log);
    botui.message.add({
      delay:500,
      loading:true,
