...
function ishiDb(targetScore, actualScore, satisfaction){
  var date = new Date(2020, 5, 5, 0, 0, 0, 0); // 月は0~11
...
function repoTime(){
  botui.message.add({
    delay:100,
    loading:true,
    content:'第３回の授業内容の学習に要した時間を分単位で記入してね。'
  }).then(function(){
    log = '第３回の授業内容の学習に要した時間を分単位で記入してね。';
    addBot(log);
...
//     delay:100,
//     loading:true,
//     content:'eラーニング教材の第３回演習問題(24問)を全て回答しましたか？'
//   }).then(function(){
//     log = 'eラーニング教材の第３回演習問題(24問)を全て回答しましたか？';
//     addBot(log);
//     return botui.action.button({
//       autoHide:false,
//       delay:10,
...
function question3(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'今回学習した「命題論理・集合③(背理法、数学的帰納法)」の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）'
  }).then(function(){
    log = '今回学習した「命題論理・集合③(背理法、数学的帰納法)」の中で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:1000,
