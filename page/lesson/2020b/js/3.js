...
function ishiDb(targetScore, actualScore, satisfaction, reflection){
  var date = new Date(2020, 9, 15, 0, 0, 0, 0); // 月は0~11
...
function question3(){
  botui.message.add({
    delay:1000,
    loading:true,
    content:'今回学習してきた内容で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）'
  }).then(function(){
    log = '今回学習してきた内容で、まだ理解ができていないと感じる箇所はありますか？（確認テストやeラーニングで不正解だった部分を理解しきれていない部分がありますか？）';
    addBot(log);
    return botui.action.button({
      autoHide:false,
      delay:1000,
...
function ishi_reflection2(executing){
  botui.message.add({
    delay:500,
    loading:true,
    content:'あなたの第３回授業の計画実施率は' + executing + '%でした。'
  }).then(function(){
    log = 'あなたの第３回授業の計画実施率は' + executing + '%でした。';
    addBot(log);
    botui.message.add({
      delay:500,
      loading:true,
